// 试产问题跟踪组件
const TrialIssuesPage = ({ projectName, onBack, onSave }) => {
  const TRIAL_PHASES = ['DV1', 'DV2', 'PV']
  const [activePhase, setActivePhase] = useState('DV1')
  const [issues, setIssues] = useState(() => {
    const saved = localStorage.getItem('tws-trial-issues')
  ...
  const phaseIssues = issues.filter(i => i.phase === activePhase && i.projectName === projectName)

  const saveIssues = (newIssues) => {
    setIssues(newIssues)
    localStorage.setItem('tws-trial-issues', JSON.stringify(newIssues))
  }
